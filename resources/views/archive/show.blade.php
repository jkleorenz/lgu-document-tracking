...
<div class="container-fluid">
    <div class="mb-4">
        ...
        <div class="d-flex justify-content-between align-items-start">
            <div>
                <h2 class="fw-bold"><i class="bi bi-archive"></i> {{ $document->title }}</h2>
            ...
            <div class="d-flex gap-2">
                ...
                <form method="POST" action="{{ route('archive.restore', $document) }}">
                    @csrf
                    <button type="submit" class="btn btn-success" onclick="return confirm('Restore this document from archive?')">
                ...
                <form method="POST" action="{{ route('archive.destroy', $document) }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger" onclick="return confirm('⚠️ PERMANENTLY DELETE this document?\n\nThis action CANNOT be undone!\n\nDocument: {{ $document->document_number }}')">
                    ...
                    <div class="d-flex mb-3">
                        ...
                        <div class="border-start border-2 ps-3 flex-grow-1">
                            ...
                            <div class="text-muted small">
                                by <strong>{{ $log->updatedBy->name }}</strong>
                                @if($log->remarks)
                                <br>{{ $log->remarks }}
                                @endif
                            </div>
