                    ...
                    <table class="table table-borderless">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <span class="badge bg-{{ $displayStatus == 'Approved' ? 'success' : ($displayStatus == 'Return' ? 'danger' : ($displayStatus == 'Received' ? 'success' : ($displayStatus == 'Retrieved' ? 'info' : ($displayStatus == 'Pending' ? 'warning' : ($displayStatus == 'Pending Verification' ? 'warning' : ($displayStatus == 'Rejected' ? 'danger' : 'info')))))) }}">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body text-center">
                    ...
                    @php
                        ...
                        $qrServeUrl = route('documents.qr-code', $document);

                        $qrCodeExists = (bool) $document->qr_code_path;
                    ...
                    <img 
                        ...
                        src="{{ $qrServeUrl }}?v={{ $document->updated_at->timestamp }}" 

                        data-qr-src="{{ $qrServeUrl }}"
                    ...
                    <div class="alert alert-warning mb-0" id="qr-code-missing">

                        <i class="bi bi-exclamation-triangle"></i> QR Code not available. Attempting to regenerate...
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const qrMissing = document.getElementById('qr-code-missing');



    if (qrCodeImage) {

        const baseUrl = qrCodeImage.dataset.qrSrc;
        ...
        qrCodeImage.addEventListener('error', function() {
            ...
            if (qrCodeImage.dataset.retry === 'regenerated') {
            ...
            qrCodeImage.dataset.retry = 'regenerated';

            if (qrError) qrError.style.display = 'none';

            qrCodeImage.style.display = 'block';

            qrCodeImage.src = `${baseUrl}?regenerate=1&v=${Date.now()}`;
        ...
        // Attempt auto-regeneration when the QR path was missing

        fetch(`{{ route('documents.qr-code', $document) }}?regenerate=1`, { cache: 'no-store' })
            ...
            .then(response => {
                ...
                if (response.ok) {
                    ...
                    qrMissing.innerHTML = '<i class="bi bi-exclamation-triangle"></i> QR Code could not be regenerated automatically. Please refresh later.';
