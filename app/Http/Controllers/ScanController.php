...
class ScanController extends Controller
{
    ...
    {
        // Get all departments for return dropdown
        ...
        return view('scan.index', compact('departments'));
    }
    ...
    public function returnDocument(Request $request)
    {
        $validated = $request->validate([
            ...
            'return_department' => ['nullable', 'exists:departments,id'],
        ]);
        ...
        DB::beginTransaction();
        try {
            // Determine the target return department
            $lastDepartmentId = null;
            $lastDepartmentName = 'Unknown Department';
            $lastHandlerId = null;
            
            // Check if a specific return department was selected
            if (!empty($validated['return_department'])) {
                // Use the selected department from the dropdown
                $lastDepartmentId = $validated['return_department'];
                $targetDepartment = Department::find($lastDepartmentId);
                if ($targetDepartment) {
                    $lastDepartmentName = $targetDepartment->name;
                } else {
                    throw new \Exception("Selected department (ID: {$lastDepartmentId}) not found.");
                }
            } else {
                // Auto-detect: Find the last department that handled this document (before current department)
            ...
            // Return document to the selected or auto-detected department
